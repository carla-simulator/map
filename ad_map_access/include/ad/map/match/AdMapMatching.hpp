 ...
 * Therefore, map matching always returns a std::multimap of map matched positions ordered by their probabilities.
 *
 * Besides the actual position, there are additional input parameters influencing the probabilistic calculation.
 * Observed heading or possible routes of a vehicle/object can be provided to influence the map matching result.
 */
class AdMapMatching
{
public:
  ...
  {
    return mRouteHintFactor;
  }

  /**
   * @brief add a hint for the heading of the vehicle/object
   *
   * It is possible to provide hints on the heading of the vehicle/object to be considered on map matching.
   * This increases the map matching probabilities for lanes with respective direction.
   * The matches are multiplied with a headingFactor:
   ...
   * This function makes use of the ENUReferencePoint of the provided ENUObjectPosition.
   * The dimensions of the ENUObjectPosition is not required.
   */
  MapMatchedPositionConfidenceList getMapMatchedPositions(ENUObjectPosition const &enuObjectPosition,
                                                          physics::Distance const &distance,
