...
AdMapAccess &AdMapAccess::getInitializedInstance()
{
  ...
  if (!singleton.mConfigFileHandler.isInitialized() && !singleton.mInitializedFromStore)
  {
    throw std::runtime_error("Instance not initialized. Forgot to call AdMapAccess::initialize?");
  }
  return singleton;
}
...
bool AdMapAccess::initialize(std::string const &configFileName)
{
  ...
  {
    mLogger->error("AdMapAccess::initialize(config) failed; already initialized from store");
    return false;
  }
  if (mConfigFileHandler.isInitialized())
  {
    ...
    {
      mLogger->error(
        "AdMapAccess::initialize(config) with file {} failed; already initialized with different config: {}",
        configFileName,
  ...
  {
    mLogger->warn("AdMapAccess::initialize(config) Failed to open {}", configFileName);
    return false;
  }

  mLogger->info("AdMapAccess::initialize(config) Successfully opened {}", configFileName);
  if (!readMap(mConfigFileHandler.adMapEntry().filename))
  {
    mLogger->warn("Unable to read map {}", mConfigFileHandler.adMapEntry().filename);
    reset();
    return false;
  }
                                                 ...
                                                 landmark::TrafficLightType const defaultTrafficLightType)
{
  ...
  {
    mLogger->error(
      "AdMapAccess::initializeFromOpenDriveContent() failed; already initialized with different config: {}",
      mConfigFileHandler.configFileName());
    return false;
  }
  ...
  {
    mLogger->error("AdMapAccess::initializeFromOpenDriveContent() failed; error parsing OpenDRIVE content");
  }
  return result;
}
...
bool AdMapAccess::initialize(Store::Ptr store)
{
  ...
  if (mConfigFileHandler.isInitialized())
  {
    mLogger->error("AdMapAccess::initialize(store) failed; already initialized with different config {}",
                   mConfigFileHandler.configFileName());
    return false;
  }
  ...
  if (mInitializedFromStore)
  {
    if (mStore != store)
    {
      mLogger->error("AdMapAccess::initialize(store) failed; already initialized with different store object");
      return false;
    }
    else
    {
