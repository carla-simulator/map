...
inline std::istream &operator>>(std::istream &input, ::ad::physics::Distance &distance)
{
  double dummy;
  if (input >> dummy)
  {
...
inline std::istream &operator>>(std::istream &input, ::ad::map::point::Latitude &latitude)
{
  double dummy;
  if (input >> dummy)
  {
...
inline std::istream &operator>>(std::istream &input, ::ad::map::point::Longitude &longitude)
{
  double dummy;
  if (input >> dummy)
  {
...
inline std::istream &operator>>(std::istream &input, ::ad::map::point::Altitude &altitude)
{
  double dummy;
  if (input >> dummy)
  {
...
bool MapConfigFileHandler::isInitializedWithFilename(std::string const &configFileName) const
{
  auto path = boost::filesystem::canonical(boost::filesystem::path(configFileName));
  auto const isEqual = (path.string() == mConfigFileName);

  if (!isEqual)
  {
    access::getLogger()->warn("AdMapAccess already initialized with a different config. Present {}, Requested {}",
                              mConfigFileName,
                              path.string());
  }
...
void MapConfigFileHandler::updateFilenameAndPath(std::string const &configFileName)
{
  auto path = boost::filesystem::canonical(boost::filesystem::path(configFileName));
...
bool MapConfigFileHandler::parseConfigFile(std::string const &configFileName)
{
  ...
  try
  {
    ...
    {
      access::getLogger()->error("Cannot open config file for reading: {}", configFileName);
      return false;
    }
    ...
    // we need the correct mBaseDir before we can construct any filename for the maps
    ...
    if (vm.count("ADMap.map"))
    {
      ...
      {
        access::getLogger()->error("Given map file {} seems not to be located below current configuration directory {} "
    ...
    if (vm.count("POI.poi"))
    {
      ...
      for (auto const &entry : entries)
      {
        ...
        {
          access::getLogger()->warn("Invalid  POI poi entry in config file: {}, Entry: {}", configFileName, entry);
          return false;
        }
      }
    }
    ...
    if (vm.count("ENUReference.default"))
    {
      ...
      if (!parseENUReference(entry))
      {
        access::getLogger()->warn(
          "Invalid default ENU reference entry in config file: {}, Entry: {}", configFileName, entry);
        return false;
      }
    }
  }
  ...
  {
    access::getLogger()->error("Error while parsing {}, Reason: {}", configFileName, e.what());
    return false;
  }
  return true;
}
...
MapEntry const &MapConfigFileHandler::adMapEntry() const
{
  return mAdMapEntry;
}
