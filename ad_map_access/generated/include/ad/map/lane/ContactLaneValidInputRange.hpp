...
inline bool withinValidInputRange(::ad::map::lane::ContactLane const &input, bool const logErrors = true)
{
  ...
  inValidInputRange = withinValidInputRange(input.to_lane, logErrors)
    && withinValidInputRange(input.location, logErrors) && withinValidInputRange(input.types, logErrors)
    && withinValidInputRange(input.restrictions, logErrors) && withinValidInputRange(input.landmark_id, logErrors);
