 ...
 * Be aware: It is independent from the logical nominal driving direction of a lane.
 */
struct ParaPoint
{
  /*!
   ...
   */
  using Ptr = std::shared_ptr<ParaPoint>;

  /*!
   * \brief Smart pointer on constant ParaPoint
   */
  using ConstPtr = std::shared_ptr<ParaPoint const>;
