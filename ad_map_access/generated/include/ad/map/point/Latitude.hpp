...
#define _AD_MAP_POINT_LATITUDE_OPERATOR_BASE_TYPE_ 0
#else
/*!
...
#define _AD_MAP_POINT_LATITUDE_OPERATOR_BASE_TYPE_ 1
#endif
...
public:
  ...
  Latitude operator+(const Latitude &other) const
  {
    ensureValid();
    other.ensureValid();
    Latitude result(mLatitude + other.mLatitude);
  ...
  {
    ensureValid();
    Latitude result(mLatitude * scalar);
    result.restrictToLimitsAndEnsureValid();
    return result;
  }
  ...
  Latitude operator/(const double &scalar) const
  {
    ...
    Latitude result(operator/(scalarLatitude));
    result.restrictToLimitsAndEnsureValid();
    return result;
  }
  ...
  /*!
   * \returns \c true if the Latitude is a normal value
   *
   * An Latitude value is defined to be normal if:
...
#if _AD_MAP_POINT_LATITUDE_OPERATOR_BASE_TYPE_
  /*!
   * \brief conversion to base type: double
   *
   * \note the conversion to the base type removes the physical unit.
  ...
  bool isValid() const
  {
    return isNormal() && (cMinValue <= mLatitude) && (mLatitude <= cMaxValue);
  }
  ...
  void ensureValid() const
  {
    if (!isValid())
    {
      spdlog::info("ensureValid(::ad::map::point::Latitude)>> {} value out of range",
  ...
  void ensureValidNonZero() const
  {
    ...
    {
      spdlog::info("ensureValid(::ad::map::point::Latitude)>> {} value is zero",
                   *this); // LCOV_EXCL_BR_LINE
#if (AD_MAP_POINT_LATITUDE_THROWS_EXCEPTION == 1)
  ...
  /**
   * @brief if possible restrict the Latitude to it's defined limits
   *
   * If the Latitude isNormal(), but exceeds the defined limits, it is restricted to its limits.
   * If Latitude::isNormal() returns \c false an std::out_of_range() exception is thrown.
  ...
  void restrictToLimitsAndEnsureValid()
  {
    if (isNormal())
    {
      if (mLatitude < cMinValue)
      {
        // mitigate exceeding the minimum
        spdlog::info("restrictToLimits(::ad::map::point::Latitude)>> {} value is smaller than allowed minimum {}. "
                     "Restrict to minimum value.",
                     *this,
                     getMin()); // LCOV_EXCL_BR_LINE
        mLatitude = cMinValue;
      }
      else if (mLatitude > cMaxValue)
      {
        // mitigate exceeding the maximum
        spdlog::info("restrictToLimits(::ad::map::point::Latitude)>> {} value is larger than allowed maximum {}. "
    ...
    {
      spdlog::info("restrictToLimits(::ad::map::point::Latitude)>> {} value out of range", *this); // LCOV_EXCL_BR_LINE
#if (AD_MAP_POINT_LATITUDE_THROWS_EXCEPTION == 1)
      throw std::out_of_range("Latitude value out of range"); // LCOV_EXCL_BR_LINE
#endif
    }
...
{
  ::ad::map::point::Latitude const result(std::fabs(other.mLatitude));
  return result;
}
...
{
  return to_string(value.mLatitude);
}
} // namespace std
 ...
 */
template <> struct fmt::formatter<::ad::map::point::Latitude> : formatter<string_view>
{
  template <typename FormatContext> auto format(::ad::map::point::Latitude const &value, FormatContext &ctx)
  {
