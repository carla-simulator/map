...
#define _AD_PHYSICS_WEIGHT_OPERATOR_BASE_TYPE_ 0
#else
/*!
...
#define _AD_PHYSICS_WEIGHT_OPERATOR_BASE_TYPE_ 1
#endif
...
public:
  ...
  Weight operator+(const Weight &other) const
  {
    ensureValid();
    other.ensureValid();
    Weight result(mWeight + other.mWeight);
  ...
  Weight operator/(const double &scalar) const
  {
    ...
    Weight result(operator/(scalarWeight));
    result.restrictToLimitsAndEnsureValid();
    return result;
  }
  ...
  /*!
   * \returns \c true if the Weight is a normal value
   *
   * An Weight value is defined to be normal if:
...
#if _AD_PHYSICS_WEIGHT_OPERATOR_BASE_TYPE_
  /*!
   * \brief conversion to base type: double
   *
   * \note the conversion to the base type removes the physical unit.
  ...
  bool isValid() const
  {
    return isNormal() && (cMinValue <= mWeight) && (mWeight <= cMaxValue);
  }
  ...
  void ensureValid() const
  {
    if (!isValid())
    {
      spdlog::info("ensureValid(::ad::physics::Weight)>> {} value out of range",
  ...
  void ensureValidNonZero() const
  {
    ...
    {
      spdlog::info("ensureValid(::ad::physics::Weight)>> {} value is zero",
                   *this); // LCOV_EXCL_BR_LINE
#if (AD_PHYSICS_WEIGHT_THROWS_EXCEPTION == 1)
  ...
  /**
   * @brief if possible restrict the Weight to it's defined limits
   *
   * If the Weight isNormal(), but exceeds the defined limits, it is restricted to its limits.
   * If Weight::isNormal() returns \c false an std::out_of_range() exception is thrown.
  ...
  void restrictToLimitsAndEnsureValid()
  {
    if (isNormal())
    {
      if (mWeight < cMinValue)
      {
        // mitigate exceeding the minimum
        spdlog::info("restrictToLimits(::ad::physics::Weight)>> {} value is smaller than allowed minimum {}. Restrict "
                     "to minimum value.",
                     *this,
                     getMin()); // LCOV_EXCL_BR_LINE
        mWeight = cMinValue;
      }
      else if (mWeight > cMaxValue)
      {
        // mitigate exceeding the maximum
        spdlog::info("restrictToLimits(::ad::physics::Weight)>> {} value is larger than allowed maximum {}. Restrict "
                     "to maximum value.",
                     *this,
    ...
    {
      spdlog::info("restrictToLimits(::ad::physics::Weight)>> {} value out of range", *this); // LCOV_EXCL_BR_LINE
#if (AD_PHYSICS_WEIGHT_THROWS_EXCEPTION == 1)
      throw std::out_of_range("Weight value out of range"); // LCOV_EXCL_BR_LINE
#endif
    }
...
{
  ::ad::physics::Weight const result(std::fabs(other.mWeight));
  return result;
}
...
{
  return to_string(value.mWeight);
}
} // namespace std
 ...
 */
template <> struct fmt::formatter<::ad::physics::Weight> : formatter<string_view>
{
  template <typename FormatContext> auto format(::ad::physics::Weight const &value, FormatContext &ctx)
  {
