...
#define _AD_PHYSICS_DISTANCESQUARED_OPERATOR_BASE_TYPE_ 0
#else
/*!
...
#define _AD_PHYSICS_DISTANCESQUARED_OPERATOR_BASE_TYPE_ 1
#endif
...
public:
  ...
  DistanceSquared operator+(const DistanceSquared &other) const
  {
    ensureValid();
    other.ensureValid();
    DistanceSquared result(mDistanceSquared + other.mDistanceSquared);
  ...
  {
    ensureValid();
    DistanceSquared result(mDistanceSquared * scalar);
    result.restrictToLimitsAndEnsureValid();
    return result;
  }
  ...
  DistanceSquared operator/(const double &scalar) const
  {
    ...
    DistanceSquared result(operator/(scalarDistanceSquared));
    result.restrictToLimitsAndEnsureValid();
    return result;
  }
  ...
  {
    ensureValid();
    DistanceSquared result(-mDistanceSquared);
  ...
  /*!
   * \returns \c true if the DistanceSquared is a normal value
   *
   * An DistanceSquared value is defined to be normal if:
...
#if _AD_PHYSICS_DISTANCESQUARED_OPERATOR_BASE_TYPE_
  /*!
   * \brief conversion to base type: double
   *
   * \note the conversion to the base type removes the physical unit.
  ...
  bool isValid() const
  {
    return isNormal() && (cMinValue <= mDistanceSquared) && (mDistanceSquared <= cMaxValue);
  }
  ...
  void ensureValid() const
  {
    if (!isValid())
    {
      spdlog::info("ensureValid(::ad::physics::DistanceSquared)>> {} value out of range",
  ...
  void ensureValidNonZero() const
  {
    ...
    {
      spdlog::info("ensureValid(::ad::physics::DistanceSquared)>> {} value is zero",
                   *this); // LCOV_EXCL_BR_LINE
#if (AD_PHYSICS_DISTANCESQUARED_THROWS_EXCEPTION == 1)
  ...
  /**
   * @brief if possible restrict the DistanceSquared to it's defined limits
   *
   * If the DistanceSquared isNormal(), but exceeds the defined limits, it is restricted to its limits.
   * If DistanceSquared::isNormal() returns \c false an std::out_of_range() exception is thrown.
  ...
  void restrictToLimitsAndEnsureValid()
  {
    if (isNormal())
    {
      if (mDistanceSquared < cMinValue)
      {
        // mitigate exceeding the minimum
        spdlog::info("restrictToLimits(::ad::physics::DistanceSquared)>> {} value is smaller than allowed minimum {}. "
                     "Restrict to minimum value.",
                     *this,
                     getMin()); // LCOV_EXCL_BR_LINE
        mDistanceSquared = cMinValue;
      }
      else if (mDistanceSquared > cMaxValue)
      {
        // mitigate exceeding the maximum
        spdlog::info("restrictToLimits(::ad::physics::DistanceSquared)>> {} value is larger than allowed maximum {}. "
    ...
    {
      spdlog::info("restrictToLimits(::ad::physics::DistanceSquared)>> {} value out of range",
                   *this); // LCOV_EXCL_BR_LINE
#if (AD_PHYSICS_DISTANCESQUARED_THROWS_EXCEPTION == 1)
      throw std::out_of_range("DistanceSquared value out of range"); // LCOV_EXCL_BR_LINE
#endif
...
{
  ::ad::physics::DistanceSquared const result(std::fabs(other.mDistanceSquared));
  return result;
}
...
{
  return os << _value.mDistanceSquared;
}
...
{
  return to_string(value.mDistanceSquared);
}
} // namespace std
 ...
 */
template <> struct fmt::formatter<::ad::physics::DistanceSquared> : formatter<string_view>
{
  template <typename FormatContext> auto format(::ad::physics::DistanceSquared const &value, FormatContext &ctx)
  {
